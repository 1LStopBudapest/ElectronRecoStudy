...
year = "2018"
if "2016pre" in options.zdata:
    year = "2016preVFP"
elif "2016post" in options.zdata:
    year = "2016postVFP"
elif "2017" in options.zdata:
    year = "2017"
...
def makeEfficiencyPlot(filename, jpsi_data_integrals, jpsi_data_integral_sigmas, jpsi_data_integral_sigmas_bksyst, jpsi_data_integral_sigmas_rangesyst, jpsi_mc_integrals, jpsi_mc_integral_sigmas, jpsi_mc_integral_sigmas_bksyst, jpsi_mc_integral_sigmas_rangesyst, jpsi_ptbins_center_fit, jpsi_binw_fit, z_data_integrals, z_data_integral_sigmas, z_data_integral_sigmas_bksyst, z_data_integral_sigmas_rangesyst, z_mc_integrals, z_mc_integral_sigmas, z_mc_integral_sigmas_bksyst, z_mc_integral_sigmas_rangesyst, z_ptbins_center_fit, z_binw_fit):
    ...
    ptbins = np.array([3.0,5.0,12.0,20.0,50.0,100.0])
    etabins = np.array([0, 1.4442, 1.566, 2.5])


    outfile = ROOT.TFile( 'combEle_SF_'+year+'.root', 'RECREATE')
    output_histo = ROOT.TH2F("comb_el_SF_2D_VetoWP_BDT2p5_"+year,"comb_el_SF_2D_VetoWP_BDT2p5_"+year,len(ptbins)-1, ptbins, len(etabins)-1, etabins)
    error_histo = ROOT.TH2F("comb_el_SFerror_2D_VetoWP_BDT2p5_"+year,"comb_el_SFerror_2D_VetoWP_BDT2p5_"+year,len(ptbins)-1, ptbins, len(etabins)-1, etabins)

    output_histo.GetXaxis().SetTitle("pt [GeV]")
    output_histo.GetYaxis().SetTitle("abs(eta)")
    error_histo.GetXaxis().SetTitle("pt [GeV]")
    error_histo.GetYaxis().SetTitle("abs(eta)")

    ROOT.gStyle.SetPaintTextFormat("4.1f")
    output_histo.SetOption("colz text89 e")
    error_histo.SetOption("COLZ,text")

    output_histo.SetStats(0)
    error_histo.SetStats(0)


    output_histo.SetBinContent(1,1,jpsi_scale_factor_endcap[0])
    output_histo.SetBinContent(1,3,jpsi_scale_factor_barrel[0])
    output_histo.SetBinError(1,1,jpsi_scale_factor_endcap_error[0])
    output_histo.SetBinError(1,3,jpsi_scale_factor_barrel_error[0])

    error_histo.SetBinContent(1,1,jpsi_scale_factor_endcap_error[0])
    error_histo.SetBinContent(1,3,jpsi_scale_factor_barrel_error[0])


    for i in range(len(z_scale_factor_barrel)):
        output_histo.SetBinContent(i+2,1,z_scale_factor_endcap[i])
        output_histo.SetBinContent(i+2,3,z_scale_factor_barrel[i])
        output_histo.SetBinError(i+2,1,z_scale_factor_endcap_error[i])
        output_histo.SetBinError(i+2,3,z_scale_factor_barrel_error[i])

        error_histo.SetBinContent(i+2,1,z_scale_factor_endcap_error[i])
        error_histo.SetBinContent(i+2,3,z_scale_factor_barrel_error[i])


    outfile.WriteObject(output_histo, "comb_el_SF_2D_VetoWP_BDT2p5_"+year)
    # outfile.WriteObject(error_histo, "comb_el_SFerror_2D_VetoWP_BDT2p5_"+year)
